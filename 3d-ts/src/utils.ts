 ...
 * @returns
 */
export function isArrayWithHexColors(arr: string[] | [string, string][]) {
 ...
 */
export function getShadowColor(color: string, step) {
  let c = getColor(color);
  let { red, blue, green } = c;
  const s = 0.8;
  const r = parseInt((red * s).toString()),
    g = parseInt((green * s).toString()),
    b = parseInt((blue * s).toString());
  const rr = (r - red) / step,
    gg = (g - green) / step,
    bb = (b - blue) / step;
  ...
  for (let i = 0; i < step; i++) {
    list.push(
      `rgb(${parseInt((red + i * rr).toString())},${parseInt(
        (green + i * gg).toString()
      )},${parseInt((blue + i * bb).toString())})`
    );
  }
  return list;
}
...
//转化为16位进制
export function to16(value: number) {
  if (value >= 0 && value <= 255) {
    ...
    if (value <= 9) {
      ...
      let shi: string | number = parseInt((value / 16).toString());
      let ge = value % 16;
 ...
 * @returns
 */
export function isColorArray(arr: string[] | [string, string][]) {
  if (!Array.isArray(arr)) {
    return false;
  }
